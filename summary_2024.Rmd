...
```{r}
# ensure maps work / image work
# move data dictionary to appendix - with links back and forth
# shorten Scenario quote (ellipsis / summarize)
# save plot(s)
# final proofread
# add links for scratch pad, connect 2019 and 2024
...
df %>%
  ...
  theme_minimal() +
  labs(title = "Rides per Hour of Day by Customer Type and Day of Week",
...
```{r, echo=FALSE, message=FALSE, warning=FALSE}
# plot_leaflet_heatmap(trips_from_top_start_members, "start_lng", "start_lat", "Top 10 Stations (Members)")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
# plot_leaflet_heatmap(trips_from_top_start_casual, "start_lng", "start_lat", "Top 10 Stations (Casuals)")
```
...
How do annual members and casual riders use Cyclistic bikes differently? Base the analysis on Cyclistic historical bike trip data. The information will be used to design marketing strategies aimed at converting casual riders into annual members.
...
The analysis incorporates cleaned ride data from all of 2024, detailing trip durations, ride frequencies, station destinations, and date-time information.
...
data_dict <- data.frame(
  Column = c("ride_id", "started_at", "ended_at", "rideable_type", 
  ...
  Description = c("Unique ride identifier.", 
...
Google provided a starter template to clean the data (see [here](https://github.com/johbry17/Cyclistic-A-Google-Data-Analytics-Capstone/blob/main/data_cleaning_v1.R), covering only Q1 2019 and Q1 2020. The template was substantially modified to clean all of 2024 (see [here](https://github.com/johbry17/Cyclistic-A-Google-Data-Analytics-Capstone/blob/main/data_cleaning_v3.R)).
...
Further insights are available [here](https://github.com/johbry17/Cyclistic-A-Google-Data-Analytics-Capstone/blob/main/scratch_pad_2024.Rmd).
