...
Yup. Missing stations are electric bikes without a docking station.
...
```{r}
# Count distinct start_station_id values
...
The mean is higher than the 3rd quartile. Data set skews. Makes sense, as many commuter rides are quick, but leisure rides can last a while. And sometimes people just dump the bike in a ditch so it appears to be a long ride.
...
# bar chart: top start stations
df %>%
  count(start_station_name, member_casual, sort = TRUE, na.rm = TRUE) %>%
  group_by(member_casual) %>%
  top_n(20, n) %>%
  ggplot(aes(x = reorder(start_station_name, n), y = n, fill = member_casual)) +
  geom_col() +
