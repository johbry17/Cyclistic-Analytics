---
title: "Cyclistic - Google Data Analytics Capstone"
...
**started_at**  
&nbsp;&nbsp;&nbsp;&nbsp; Timestamp indicating when the ride started (YYYY-MM-DD HH:MM:SS).  
...
**member_casual**  
&nbsp;&nbsp;&nbsp;&nbsp; Type of rider (member for annual subscribers, casual for non-members).  
...
**season**  
&nbsp;&nbsp;&nbsp;&nbsp; Season in which the ride took place (Winter, Spring, Summer, Fall).  
...
Google linked to [data](https://divvy-tripdata.s3.amazonaws.com/index.html) provided by Lyft and Divvy from their Chicago operations.
...
Members riders go to the "Loop". They use Cyclistic for commuting to places of employment.
...
member_table <- member_peak_stations %>%
  rename(Member_Rides = n) %>%
  select(start_station_name, Member_Rides) %>%  # exclude index
  kable("html", caption = "Top 10 Member Peak Stations") %>%
  kable_styling(bootstrap_options = c("striped", "hover"))

casual_table <- casual_peak_stations %>%
  rename(Casual_Rides = n) %>%
  select(start_station_name, Casual_Rides) %>%  # exclude index
...
Compare the above map, especially the top casual destinations, with the Chicago sightseeing map.
